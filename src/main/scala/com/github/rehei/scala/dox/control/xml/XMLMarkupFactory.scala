...
trait XMLMarkupFactory {
  ...
  def H1(name: String) = XMLHeadline(referenceFactory.generic(), name, 1, headline => <h1 id={ headline.id }>{ headline.name }</h1>)
  def H2(name: String) = XMLHeadline(referenceFactory.generic(), name, 2, headline => <h2 id={ headline.id }>{ headline.name }</h2>)
  def H3(name: String) = XMLHeadline(referenceFactory.generic(), name, 3, headline => <h3 id={ headline.id }>{ headline.name }</h3>)
  def H4(name: String) = XMLHeadline(referenceFactory.generic(), name, 4, headline => <h4 id={ headline.id }>{ headline.name }</h4>)
